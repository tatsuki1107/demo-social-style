...
def questions(request):
    ...
    if request.method == 'OPTIONS':
        ...
        questions = [{"questions":"聞くよりも話すほうが好きだ","select-type":2,"pos":"X"},{"questions":"自分はせっかちなほうだ","select-type":2,"pos":"X"},
                     {"questions":"人をまとめるのが得意だ","select-type":2,"pos":"X"},{"questions":"相手の目を見て話す方だ","select-type":2,"pos":"X"},
                     {"questions":"人を動かすことができる","select-type":2,"pos":"X"},{"questions":"思ったことがすぐ口から出てしまう方だ","select-type":2,"pos":"X"},
                     {"questions":"組体操より、かけっこが好きだ","select-type":2,"pos":"X"},{"questions":"他人と話すのが好きだ","select-type":2,"pos":"X"},
                     {"questions":"話し合いでは、みんなの意見を尊重するより自分の意見を主張する方だ","select-type":2,"pos":"X"},{"questions":"自分は感情豊かだ","select-type":2,"pos":"X"},
                     {"questions":"「元気だね」とよく言われる","select-type":2,"pos":"X"},{"questions":"仕事よりプライベートを大事にしたい","select-type":2,"pos":"X"},
                     {"questions":"自分はクールだと言われない","select-type":2,"pos":"X"},{"questions":"自分の気持ちを表すのが得意だ","select-type":2,"pos":"X"},
                     {"questions":"自分は陽気な方だと思う","select-type":2,"pos":"X"},{"questions":"データより人の意見を信じる方だ","select-type":2,"pos":"X"},
                     {"questions":"自分は感情豊かで涙もろいほうと思う","select-type":2,"pos":"X"},{"questions":"会話では、抑揚つけて話す方だ","select-type":2,"pos":"X"}]
        howMany = len(questions)

        if howMany <= 20:
            return_questions = json.dumps(questions, indent=0, ensure_ascii=False)
        else:
            num = set()
            while len(num) < 20:
    ...
    if request.method == 'OPTIONS':
        ...
        not (0 <= res["Y"] <= 100)) or (type(res["session_ID"]) != str) or (type(res["token"]) != str)):
            ...
            if res["X"] > 50 and res["Y"] > 50:
                ...
                socialStyle = 4
            try:
                Result.objects.create(User_ID=userid,SocialStyle_ID=socialStyle,X=res["X"],Y=res["Y"],date=now)
                return HttpResponse(status=200)
            except:
                return HttpResponse(status=500)
    else:
    ...
    if request.method == 'OPTIONS':
        ...
        if "time" in res:
            try:
                ...
                feature = Feature.objects.filter(SocialStyle_ID__exact=result.SocialStyle_ID).all()
                profession = Profession.objects.filter(SocialStyleID__exact=result.SocialStyle_ID).all()
                relation = Relational.objects.filter(MySocialStyle_ID__exact=result.SocialStyle_ID).all()
                socialStyle = SocialStyle.objects.get(SocialStyle_ID__exact=result.SocialStyle_ID)
            except:
                return HttpResponse(status=500)
            response_body = {}
            ...
            try:
                ...
                feature = Feature.objects.filter(SocialStyle_ID__exact=SocialStyle_ID).all()
                profession = Profession.objects.filter(SocialStyleID__exact=SocialStyle_ID).all()
                relation = Relational.objects.filter(MySocialStyle_ID__exact=SocialStyle_ID).all()
                socialStyle = SocialStyle.objects.get(SocialStyle_ID=SocialStyle_ID)
            except:
                return HttpResponse(status=500)
            response_body = {}
            response_body["Time"] = result.Date
